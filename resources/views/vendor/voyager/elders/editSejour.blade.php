...
@section('page_header')
    <h1 class="page-title">
        Modifier de chambre pour {{$elder->elder_first_name}} {{$elder->elder_last_name}} 
    </h1>
        ...
        <div class="row">
            <div class="col-md-12">
                <div class="panel panel-bordered container-fluid" style="padding-bottom:15px; padding-top: 15px;">
                    <form  method="post" onsubmit="return validateForm()" action="{{route('editSejour')}}" class="col-md-6" enctype="multipart/form-data">
                        ...
                        <div class="form-group">
                            ...
                            <select id="typeSejour" name="typeSejour" class="form-control" style="padding-right: 20px; padding-left: 20px;">
                                <option value="Cours séjour" @if($elderRoom['0']['typeSejour'] == 'Cours séjour') selected @endif>Cours séjour</option>
                                <option value="Long séjour" @if($elderRoom['0']['typeSejour'] == 'Long séjour') selected @endif>Long séjour</option>
                                <option value="Acceuil de jour" @if($elderRoom['0']['typeSejour'] == 'Acceuil de jour') selected @endif>Acceuil de jour</option>
                            </select>
                        </div>
                        <div class="form-group">
                            <h4>Date d'entrée *</h4>
                        ...
                        <div class="form-group">
                            ...
                            <select id="chambre" name="chambre" class="form-control" style="padding-right: 20px; padding-left: 20px;">
                               @foreach ($rooms as $room)
                                <option value="{{$room->numero}}" @if($chambreNumero == $room->numero) selected @endif>{{$room->numero}}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="form-group">
                            <h4>Prix *</h4>
                            <input id="prix" type="number" class="form-control" step="any" name="prix" required >
                        </div>
                        <div class="form-group">
                            <h4>Contrat de sejour *</h4>
                            <input type="file"  name="contrat" required>
                        </div>
                        <button type="submit" class="btn btn-primary save">Modifier le séjour</button>
                        <a href="{{route('deleteSejour', ['id'=>$elder->id])}}"><button type="button" class="btn btn-danger">Suppimer Séjour</button></a>
                    </form>
                    <br>
